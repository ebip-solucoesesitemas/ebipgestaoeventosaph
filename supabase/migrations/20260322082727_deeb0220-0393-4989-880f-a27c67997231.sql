...
DECLARE
  ...
  v_valor_hora_centavos bigint;
  v_ajuda_custo_centavos bigint;
...
BEGIN
  ...
  -- Use centavos-based calculation to avoid floating point errors
  v_payment_value := 0;
  v_total_centavos := 0;
  
  IF v_rate IS NOT NULL THEN
    IF v_rate.valor_hora > 0 THEN
      v_valor_hora_centavos := ROUND(v_rate.valor_hora * 100)::bigint;
      v_total_centavos := ROUND(v_hours * v_valor_hora_centavos)::bigint;
    ELSIF v_rate.valor_evento > 0 THEN
      v_total_centavos := ROUND(v_rate.valor_evento * 100)::bigint;
    END IF;
  END IF;

  -- Add ajuda de custo in centavos
  v_ajuda_custo_centavos := ROUND(v_ajuda_custo * 100)::bigint;
  v_total_centavos := v_total_centavos + v_ajuda_custo_centavos;
  
  -- Convert back to reais
  v_payment_value := v_total_centavos / 100.0;
